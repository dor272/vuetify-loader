...
const { dirname, join, relative, sep, posix: {sep: posixSep} } = require('path')
...
Module._load = function _load (request, parent) {
  ...
  if (request.endsWith('.sass')) {
    groupStyleDependencies.add(relative(vuetifyRootPath, join(dirname(parent.filename), request))/* .split(sep).join(posixSep) */)
  }
